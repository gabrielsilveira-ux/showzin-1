...
import { Producer, ProducersListResponse } from "@/types/producers";
import {
  actionButtonClasses,
  primaryButtonStyles,
  secondaryButtonStyles,
} from "@/styles/styles";
import { formatCnpjDisplay } from "@/utils/formats";

export default function ProducersContainer() {
  const router = useRouter();
  const dispatch = useAppDispatch();
  const [search, setSearch] = useState("");
  const [appliedSearch, setAppliedSearch] = useState("");

  const { data, isLoading, isError, refetch } = useQuery<ProducersListResponse>(
    {
      queryKey: ["producers-list", appliedSearch],
      staleTime: 1000 * 60,
      queryFn: async () => {
        const response = await ProducersServer.getProducers({
          name: appliedSearch || undefined,
        });
        if (!response.isSuccess || !response.data) {
          throw new Error(
            response.error?.message ??
              "Não foi possível carregar as produtoras."
          );
        }
  ...
  const emptyMessage = useMemo(() => {
    if (appliedSearch) {
      return "Nenhuma produtora encontrada para o filtro informado.";
    }
    return "Nenhuma produtora cadastrada até o momento.";
  }, [appliedSearch]);
        ...
        <PageHeader
          headerContent={
            <div className="space-y-2">
              ...
              >
                Painel Administrativo / Produtoras
              </div>
              <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <h1 className="text-2xl font-semibold">
                    Produtoras cadastradas
                  </h1>
                  <p className="text-sm" style={{ color: COLORS.textMuted }}>
                    Gerencie as produtoras responsáveis pelos eventos.
                  </p>
                </div>
                <span
                  className="text-xs uppercase tracking-[0.26em]"
                  ...
                  <tbody className="divide-y divide-white/5">
                    {producers.map((producer: Producer) => (
                      <tr
                        ...
                        onClick={() =>
                          router.push(`/produtoras/${producer.producerId}`)
                        }
                      >
                        <td className="px-6 py-4 text-white">
                          {producer.name ?? "Sem nome"}
                        </td>
                        <td
                          className="px-6 py-4"
                          style={{ color: COLORS.textSecondary }}
                        >
                          {formatCnpjDisplay(producer.documentNumber)}
                        </td>
                        <td className="px-6 py-4">
                          <span
                            className="rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.24em]"
                            style={{
                              backgroundColor: producer.isActive
                                ? "rgba(72,255,168,0.16)"
                                : "rgba(255,214,102,0.16)",
                              color: producer.isActive
                                ? COLORS.successText
                                : "#ffd666",
                            }}
                          >
                            {producer.isActive ? "Ativa" : "Inativa"}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex justify-end gap-2">
                            <Link
                              href={`/produtoras/${producer.producerId}`}
